...
class StatisticsIJsselVechtdelta(Statistics):
    ...
    def __init__(self, settings : Settings):
        ...
        self.stochastics_slow = {"a" : self.lake_level.get_discretisation(), "q" : self.discharge.get_discretisation()}
    ...
    def calculate_probability(self, wind_direction: float, closing_situation: int = 1, given: list = []):
        ...
        kanswind = ProbabilityFunctions.probability_density(self.wind_speed.get_discretisation(), self.wind_speed.get_exceedance_probability()[:, ir]).probability
        if "u" in given:
            kanswind[:] = 1.0
        ...
        for iq in range(nq):
            ...
            for im in reversed(range(nm)):
                ...
                dichtheid_mqpiek[im, iq] = (
                    kansmeerpeil.density[im] * np.exp(-(y ** 2) / (2.0 * sigma ** 2)) / (sigma * (2.0 * np.pi) ** 0.5)
                )
