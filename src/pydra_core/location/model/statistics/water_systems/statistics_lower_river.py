...
class StatisticsLowerRiver(Statistics):
    ...
    def __init__(self, settings : Settings):
        ...
        # Europoort Barrier
        self.barrier = BarrierEuropoort(settings, self.wind_direction, self.wind_speed, self.sea_level, self.discharge)

        # Model uncertainty
        self.model_uncertainties = ModelUncertainty(settings)

        # Discrete, slow, fast stochatics
        self.stochastics_discrete = {"r" : self.wind_direction.get_discretisation(), "k" : self.barrier.k}
    ...
    def calculate_probability(self, wind_direction : float, closing_situation : int = 1, given : list = []):
        ...
        if ((wind_direction >= 0.0) and (wind_direction <= 11.25)) or ((wind_direction > 212.75) and (wind_direction <= 360.0)):
            if "q" in given:
                ...
                kansafvoer = ProbabilityFunctions.probability_density(self.discharge.get_discretisation(), 1 - self.discharge.get_exceedance_probability()).probability
            ...
            # Kans zeewaterstand
            kanszws = ProbabilityFunctions.probability_density(self.sea_level.get_discretisation(), self.sea_level.get_exceedance_probability()[:, ir]).probability[None, :, None]
            if "m" in given:
                kanszws[:] = 1.0
        ...
        # Otherwise, sector east (without sea level and Europoort)
            ...
            # Kans windsnelheid
            ir = self.wind_direction.get_discretisation().tolist().index(wind_direction)
            kanswind = ProbabilityFunctions.probability_density(self.wind_speed.get_discretisation(), self.wind_speed.get_exceedance_probability()[:, ir]).probability[:, None]
            if "u" in given:
                kanswind[:] = 1.0
    ...
    def bereken_kru_tabel(self, wind_direction : float):
        ...
        # TODO: logaritmisch?
        ovkanszws = Interpolate.inextrp1d(x = m, xp = self.sea_level.get_discretisation(), fp = self.sea_level.get_exceedance_probability()[:, ir])
        ...
        kru[~idx] = Interpolate.inextrp1d(x=self.wind_speed.get_discretisation()[~idx], xp=self.wind_speed.get_discretisation()[idx], fp=kru[idx])
