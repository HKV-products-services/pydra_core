...
class BarrierEasternScheldt(Barrier):
    def __init__(self, settings : Settings, wind_direction : DiscreteProbability, wind_speed : WindSpeed, sea_level : SeaLevel):
        # Inherit
        ...
        self.k, self.p_fail_k = FileHydraNL.read_file_ncolumns(settings.barrier_scenario_probability)
        ...
        intstr = InterpStruct(x = wind_speed.get_discretisation(), xp = self.closing_levels.u)
        levels_u = intstr.interp(fp = self.closing_levels.h_rpb, axis = self.closing_levels.input_variables.index("u"))
        
        # Dan over de zeewaterstand
        intstr = InterpStruct(x = sea_level.get_discretisation(), xp = self.closing_levels.m)
        self.h_rpb = intstr.interp(fp = levels_u, axis = self.closing_levels.input_variables.index("m"))
    ...
    def calculate_closing_probability(self, wind_direction : float, closing_situation : int) -> np.ndarray:
        ...
        if sluittype == "Noodsluiting":
            kansfalensluiting = kansfalen_bemand[ik] * kans_handmatig_m + kansfalen_onbemand[ik] * kans_noodsluit_m
        ...
        keringsituatiekansen = Interpolate.inextrp1d(x = self.h_rpb[:, :, ir, :, :], xp = self.m_closing, fp = kansfalensluiting)
