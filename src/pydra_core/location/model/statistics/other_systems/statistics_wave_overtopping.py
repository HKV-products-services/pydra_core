...
class StatisticsWaveOvertopping(Statistics):
    def __init__(self, location : Location, water_levels : np.ndarray, probability : np.ndarray):
        ...
        self.stochastics_fast = {"wlev" : water_levels, "u" : location.get_model().get_statistics().stochastics_fast['u']}
        self.stochastics_slow = location.get_model().get_statistics().stochastics_slow

        # Discretisation
        discretisation = {**self.stochastics_fast, **self.stochastics_slow, **self.stochastics_discrete}
