...
class LoadingEasternScheldt(Loading):
    ...
    def read_loading(self) -> None:
        ...
        waterlevels.loc[missing_h_index, 'h'] = np.around(waterlevels.loc[missing_h_index - 1, 'h'].to_numpy() + 
            (waterlevels.loc[missing_h_index, 'u'].to_numpy() - waterlevels.loc[missing_h_index - 1, 'u'].to_numpy()) * 
            ((waterlevels.loc[missing_h_index + 1, 'h'].to_numpy() - waterlevels.loc[missing_h_index - 1, 'h'].to_numpy()) / 
            (waterlevels.loc[missing_h_index + 1, 'u'].to_numpy() - waterlevels.loc[missing_h_index - 1, 'u'].to_numpy())), 4)
        ...
        for n, row in waveconditions[waveconditions['hs'].isna()].iterrows():
            ...
            waveconditions.loc[n, 'tspec'] = round(float(interp1d(grid['u'], grid['tspec'], fill_value="extrapolate")(ws)), 4)
