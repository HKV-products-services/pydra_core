...
class Settings():
    ...
    def __determine_sea_level_statistics_points(self):
        ...
        """
        # Coast
        if self.watersystem in [WaterSystem.COAST_SOUTH, WaterSystem.COAST_CENTRAL, WaterSystem.COAST_NORTH]:
            ...
            if CommonFunctions.position_from_line('north', (self.x_coordinate, self.y_coordinate), [refpoints.loc['Den Oever'].values, refpoints.loc['Den Helder'].values]):
                subregion = 1
        ...
        # Wadden sea
        elif self.watersystem in [WaterSystem.WADDEN_SEA_WEST, WaterSystem.WADDEN_SEA_EAST]:
            ...
            # Determine the subregion
            if CommonFunctions.position_from_line('east', (self.x_coordinate, self.y_coordinate), [refpoints.loc['Lauwersoog Haven'].values, refpoints.loc['Huibertgat'].values]):
                subregion = 1
            elif CommonFunctions.position_from_line('north', (self.x_coordinate, self.y_coordinate), [refpoints.loc['Lauwersoog Haven'].values, refpoints.loc['West-Terschelling'].values]):
                subregion = 2
            elif CommonFunctions.position_from_line('east', (self.x_coordinate, self.y_coordinate), [refpoints.loc['Harlingen'].values, refpoints.loc['West-Terschelling'].values]):
                subregion = 3
            elif CommonFunctions.position_from_line('east', (self.x_coordinate, self.y_coordinate), [refpoints.loc['Den Oever'].values, refpoints.loc['West-Terschelling'].values]):
                subregion = 4
        ...
        # Western scheldt
            ...
            subsystem = np.array(["Zeewaterstand\Vlissingen\CondPovVlissingen_12u_zichtjaar2017_metOnzHeid.txt", 
    ...
    def __determine_lower_river_settings(self):
        ...
        PATH = os.path.join(os.path.split(os.path.dirname(__file__))[0], "..", "data", "settings", "lower_river_settings.shp")
        with fn.open(PATH, "r") as shp:
            ...
            for feature in shp:
                ...
                if point.within(geometry):
                    ...
                    if self.watersystem in [WaterSystem.RHINE_TIDAL, WaterSystem.EUROPOORT]:
