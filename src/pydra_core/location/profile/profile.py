...
class Profile():
    ...
    def validate_profile(self) -> bool:
        ...
        # Orientation
        if self.dike_orientation == None:
            return False

        # Crest height
        if self.dike_crest_level == None:
            return False
    ...
    def set_dike_geometry(self, dike_x_coordinates: list, dike_y_coordinates: list, dike_roughness: list = None):
        """
    ...
    def calculate_overtopping(self, water_level: Union[float, list], significant_wave_height: Union[float, list], spectral_wave_period: Union[float, list], wave_direction: Union[float, list], tp_tspec: float = 1.1, dll_settings: dict = None) -> Union[float, list]:
        """
        ...
        water_level, significant_wave_height, spectral_wave_period, wave_direction = self.transform_wave_conditions(water_level, significant_wave_height, spectral_wave_period, wave_direction, tp_tspec, force_array = True)
    ...
    def calculate_runup(self, water_level: Union[float, list], significant_wave_height: Union[float, list], spectral_wave_period: Union[float, list], wave_direction: Union[float, list], tp_tspec: float = 1.1, dll_settings: dict = None) -> Union[float, list]:
        """
    ...
    def calculate_crest_level(self, q_overtopping: float, water_level: Union[float, list], significant_wave_height: Union[float, list], spectral_wave_period: Union[float, list], wave_direction: Union[float, list], tp_tspec: float = 1.1, dll_settings: dict = None) -> Union[float, list]:
        """
        ...
        hbn = []
        for _q, _h, _hs, _tspec, _dir in zip(q_overtopping, water_level, significant_wave_height, spectral_wave_period, wave_direction):
    ...
    def transform_wave_conditions(self, water_level: Union[float, list], significant_wave_height: Union[float, list], spectral_wave_period: Union[float, list], wave_direction: Union[float, list], tp_tspec: float = 1.1, force_array: bool = False) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        """
        ...
        if self.has_foreland():
            fl = Foreland(self)
            water_level, significant_wave_height, peak_wave_period, wave_direction = fl.transform_wave_conditions(water_level, significant_wave_height, spectral_wave_period * tp_tspec, wave_direction)
            spectral_wave_period = peak_wave_period /  tp_tspec

        # Return array or floats?
        if len(water_level.ravel()) == 1 and not force_array:
            return (water_level.ravel()[0], significant_wave_height.ravel()[0], spectral_wave_period.ravel()[0], wave_direction.ravel()[0])
        else:
            return (water_level, significant_wave_height, spectral_wave_period, wave_direction)
    ...
    def draw_profile(self) -> None:
        ...
        # Foreland
        if self.foreland_x_coordinates is not None:
            plt.plot(self.foreland_x_coordinates, self.foreland_y_coordinates, color = "orange", label = "Foreland", zorder = 1)
        
        # Geometry
        for i in range(len(self.dike_x_coordinates) - 1):
            r = self.dike_roughness[i]
            plt.plot(self.dike_x_coordinates[i:i+2], self.dike_y_coordinates[i:i+2], color = "black" if r != 1.0 else "green", label = f"Roughness: {r}", zorder = 1)
        ...
        if isinstance(self.breakwater_type, Breakwater):
            ...
            if self.foreland_x_coordinates is not None:
                x_orig, y_orig = self.foreland_x_coordinates[0], self.foreland_y_coordinates[0]
            else:
            ...
            if self.breakwater_type == Breakwater.CAISSON:
                plt.fill_between([x_orig - 10, x_orig], [bw_height, bw_height], [y_orig, y_orig], color = "dimgrey", label = "Caisson", zorder=2)
            elif self.breakwater_type == Breakwater.VERTICAL_WALL:
                plt.plot([x_orig, x_orig], [y_orig, bw_height], color = "black", lw = 3, label = "Vertical wall", zorder=2)
            elif self.breakwater_type == Breakwater.RUBBLE_MOUND:
               diff = bw_height - y_orig
               plt.fill_between([x_orig - (3 * diff) - 2, x_orig - (1.5 * diff) - 2, x_orig - (1.5 * diff), x_orig], [y_orig, bw_height, bw_height, y_orig], [y_orig, y_orig, y_orig, y_orig], color = "grey", label = "Rubble mound", zorder=2)
        
        # Crest height
        plt.axhline(self.dike_crest_level, linestyle = ":", color = "grey", label = "Crest height", zorder = 1)
