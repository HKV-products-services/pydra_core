...
class Interpolate:
    ...
    def inextrp1d_log_probability(x: np.ndarray, xp: np.ndarray, fp: np.ndarray) -> np.ndarray:
        ...
        fracs[~iszero] = (np.log(x[~iszero]) - np.log(xp[intidx_log])) / (np.log(xp[intidx_log + 1]) - np.log(xp[intidx_log]))
...
class InterpStruct:
    ...
    def interp(
        ...
        # Interpolate (1-frac) * f_low + frac * f_up
        f = (1 - self.fracs.reshape(shape)) * np.take(fp, self.intidx, axis=axis) + np.take(fp, self.intidx + 1, axis=axis) * self.fracs.reshape(shape)
