...
class Interpolate:
    ...
    @staticmethod
    def inextrp1d_log_probability(x : np.ndarray, xp : np.ndarray, fp : np.ndarray) -> np.ndarray:
        """
        ...
        if not (np.diff(xp) >= 0.0).all():
            raise ValueError("[ERROR] xp-values are not monotonically increasing or decreasing")
        ...
        fracs[iszero] = (x[iszero] - xp[intidx_lin]) / (xp[intidx_lin + 1] - xp[intidx_lin])
    ...
    @staticmethod
    def triangular_interpolation(x1 : float, y1 : float, h1 : float, x2 : float, y2 : float, h2 : float, x3 : float, y3 : float, h3 : float, x : float, y : float) -> float:
        """
...
class InterpStruct:
    ...
    def __init__(self, x : np.ndarray, xp : np.ndarray):
        ...
        self.x = np.asarray(x)
        self.intidx = np.minimum(np.maximum(0, np.searchsorted(self.xp, self.x) - 1), len(self.xp) - 2)
        self.fracs = (self.x - self.xp[self.intidx]) / (self.xp[self.intidx + 1] - self.xp[self.intidx])
        

    def interp(self, fp : np.ndarray, axis : int = 0, extrapolate : bool = True, left : bool = None, right : bool = None):
        """
        ...
        if axis > fp.ndim - 1:
            raise ValueError(f"Given axis ({axis}) is higher than dimensions of fp array ({fp.ndim}).")
        ...
        # Interpolate (1-frac) * f_low + frac * f_up
        f = (1 - self.fracs.reshape(shape)) * np.take(fp, self.intidx, axis=axis) + np.take(
            fp, self.intidx + 1, axis=axis
        ) * self.fracs.reshape(shape)
    ...
    def interp_angle(self, fp : np.ndarray, axis : int = 0, extrapolate : bool =True) -> np.ndarray:
        """
